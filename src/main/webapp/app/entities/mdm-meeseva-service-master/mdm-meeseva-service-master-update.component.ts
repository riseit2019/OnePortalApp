...
export class MdmMeesevaServiceMasterUpdateComponent implements OnInit {
  isSaving = false;
  meesevaserviceids: IMdmServiceMaster[] = [];
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ mdmMeesevaServiceMaster }) => {
      ...
      this.mdmServiceMasterService
        .query({ filter: 'mdmmeesevaservicemaster-is-null' })
        .pipe(
          map((res: HttpResponse<IMdmServiceMaster[]>) => {
            return res.body || [];
          })
        )
        .subscribe((resBody: IMdmServiceMaster[]) => {
          if (!mdmMeesevaServiceMaster.meesevaServiceId || !mdmMeesevaServiceMaster.meesevaServiceId.id) {
            this.meesevaserviceids = resBody;
          } else {
            this.mdmServiceMasterService
              .find(mdmMeesevaServiceMaster.meesevaServiceId.id)
              .pipe(
                map((subRes: HttpResponse<IMdmServiceMaster>) => {
                  return subRes.body ? [subRes.body].concat(resBody) : resBody;
                })
              )
              .subscribe((concatRes: IMdmServiceMaster[]) => (this.meesevaserviceids = concatRes));
          }
        });
    });
  }
  ...
  updateForm(mdmMeesevaServiceMaster: IMdmMeesevaServiceMaster): void {
    this.editForm.patchValue({
      ...
      syspk: mdmMeesevaServiceMaster.syspk,
  ...
  trackById(index: number, item: IMdmServiceMaster): any {
    return item.id;
  }
}
