  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ mdmCitizenData }) => {
      ...
      this.mdmEmployeeMasterService
        .query({ filter: 'mdmcitizendata-is-null' })
        .pipe(
        ...
        .subscribe((resBody: IMdmEmployeeMaster[]) => {
          if (!mdmCitizenData.personId || !mdmCitizenData.personId.id) {
            this.personids = resBody;
          } else {
            this.mdmEmployeeMasterService
              .find(mdmCitizenData.personId.id)
              .pipe(
                map((subRes: HttpResponse<IMdmEmployeeMaster>) => {
                  return subRes.body ? [subRes.body].concat(resBody) : resBody;
                })
              )
              .subscribe((concatRes: IMdmEmployeeMaster[]) => (this.personids = concatRes));
          }
        });
    });
  }
