  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ mdmActivityMaster }) => {
      ...
      this.mdmOrganizationMasterService
        .query({ filter: 'mdmactivitymaster-is-null' })
        .pipe(
        ...
        .subscribe((resBody: IMdmOrganizationMaster[]) => {
          if (!mdmActivityMaster.organizationCode || !mdmActivityMaster.organizationCode.id) {
            this.organizationcodes = resBody;
          } else {
            this.mdmOrganizationMasterService
              .find(mdmActivityMaster.organizationCode.id)
              .pipe(
