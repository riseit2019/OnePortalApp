...
public class MdmServiceMasterResource {
    ...
    public List<MdmServiceMaster> getAllMdmServiceMasters(@RequestParam(required = false) String filter) {
        if ("meesevaserviceid-is-null".equals(filter)) {
            log.debug("REST request to get all MdmServiceMasters where meesevaServiceId is null");
            return StreamSupport
                .stream(mdmServiceMasterRepository.findAll().spliterator(), false)
                .filter(mdmServiceMaster -> mdmServiceMaster.getMeesevaServiceId() == null)
