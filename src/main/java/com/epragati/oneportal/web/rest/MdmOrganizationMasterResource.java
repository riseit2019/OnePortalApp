...
public class MdmOrganizationMasterResource {
    ...
    public List<MdmOrganizationMaster> getAllMdmOrganizationMasters(@RequestParam(required = false) String filter) {
        if ("organizationcode-is-null".equals(filter)) {
            log.debug("REST request to get all MdmOrganizationMasters where organizationCode is null");
            return StreamSupport
                .stream(mdmOrganizationMasterRepository.findAll().spliterator(), false)
                .filter(mdmOrganizationMaster -> mdmOrganizationMaster.getOrganizationCode() == null)
