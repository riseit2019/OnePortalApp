...
public class MdmCitizenDataResource {
    ...
    public List<MdmCitizenData> getAllMdmCitizenData(@RequestParam(required = false) String filter) {
        if ("personid-is-null".equals(filter)) {
            log.debug("REST request to get all MdmCitizenDatas where personId is null");
            return StreamSupport
                .stream(mdmCitizenDataRepository.findAll().spliterator(), false)
                .filter(mdmCitizenData -> mdmCitizenData.getPersonId() == null)
